	...
	protected void accionBotones(ActionEvent e) {
		...
		if (e.getSource() == crearTicket) {
			try {
				...
				viewInfo.setTextInfoProducte(this.productesController.mostraStockPerPantalla());
				//System.out.println( this.productesController.mostraStockPerPantalla());
		...
		if (e.getSource() == printOldTickets) {
			try {
				...
				System.out.println(ticketController.mostrarTicketsAnteriors());
			} catch (Exception e1) {
				viewInfo.setTextLabelError("Error " + e1.getMessage());
			}
		}
		...
		if (e.getSource() == printSumDinersTickets) {
			try {
				...
				System.out.println("Visualitzar el total de diners guanyats amb totes les ventes: " + ticketController.mostraDinersGuanyatAmbVentes());
